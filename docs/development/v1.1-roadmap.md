...
| **Structured event log** | `29ce646` | 121+444 | 22 |
| **Vault channel** | `9d678b1` | ~280 | 9 |
...
**Vault**: `VaultChannel(BaseChannel)` file-system watcher for Obsidian Web Clipper integration. Polls `watch_dir` for new `.md` files, routes tasks by subdirectory name (e.g. `inbox/summarize/` triggers summarize prompt). Results written back to `output_dir` and forwarded to notification channel (Telegram/Discord). State persistence via `vault_state.json` prevents reprocessing after restart. Zero external dependencies (stdlib polling).
...
v1.1 delivers five core improvements: real-time streaming UX, Discord community reach, multi-provider routing with quota management, structured event logging for operational analytics, and Vault file-watcher channel for Obsidian Web Clipper integration. Total: ~2694 lines added, 97 new tests, 523 tests passing. All remaining features are deferred until real user demand justifies the complexity.
